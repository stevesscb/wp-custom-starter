...
add_filter('login_headerurl', function () {
  return 'https://custom.com.hk';
});

// Custom Admin Login Logo Title
add_filter('login_headertext', function () {
  return 'custom web design and web development Hong Kong';
});
...
add_action('init', function () {
  register_nav_menus(array(
    'main' => __('Main Menu', 'custom'),
    'footer' => __('Footer Menu', 'custom'),
    'offcanvas' => __('Off-canvas Menu', 'custom'),
  ));
});
...
class GS_Comment extends Walker_Comment
{
  ...
  public function start_el(&$output, $comment, $depth = 0, $args = array(), $id = 0)
  {
    ...
    if (($comment->comment_type == 'pingback'  || $comment->comment_type == 'trackback') && $args['short_ping']) {
      ...
      $output .= __('Pingback:', 'custom') . ' ' . get_comment_author_link($comment) . ' ' . edit_comment_link(__('Edit', 'custom'), ' | ', '');
      $output .= '</div>';
    } else {
      ...
      $output .= '<time datetime="' .  get_comment_time('c') . '">' . sprintf(__('%1$s at %2$s', 'custom'), get_comment_date('', $comment), get_comment_time()) . '</time>';
      $output .= ' | <a href="' . esc_url(get_comment_link($comment, $args)) . '">#</a>';
      $output .= (current_user_can('edit_comment', $comment->comment_ID)) ? ' | <a href="' . get_edit_comment_link() . '">' . __('Edit', 'custom') . '</a>' : '';
      $output .= ($comment->comment_approved == '0') ? ' | ' . __('Your comment is awaiting moderation.', 'custom') : '';
      $output .= '</p>';
      $output .= '</div>';
